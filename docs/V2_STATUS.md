...
| 2 | **TODAS as mensagens passam pelo renderer** | PRONTO E TESTADO | `src/services/ai-v2/llm.ts` + `actions.ts` | A resposta do LLM (`output.reply`) passa por `renderMessage` em `llm.ts` antes de retornar ao engine. Mensagens de ação (`send_message_model`) e fallback também usam `renderMessage`. Testes em `llm.test.ts` e `actions.test.ts`. | — |
...
| 6 | **Temas: allowlist de tools aplicada no LLM** | PRONTO E TESTADO | `src/services/ai-v2/llm.ts` `buildV2ToolSet` | Apenas as tools permitidas pelo tema (ou globalmente) são expostas ao modelo. Teste em `llm.test.ts` verifica que `search_crm_records` desaparece quando o tema só permite `search_products`. | — |
| 6 | **Temas: allowlist de docs/modelos/produtos** | PRONTO E TESTADO | `src/services/ai-v2/tools.ts` | `searchV2Knowledge` filtra chunks por `allowedDocIds` do tema/agente; `listV2MessageModels` filtra por `allowedIds`. Testes em `tools.test.ts`. | `productPolicy` por tema ainda não é lido no envio de produtos. |
...
| 7 | **Function calling / tools de consulta** | PRONTO E TESTADO | `src/services/ai-v2/llm.ts` + `tools.ts` | LLM chamado com tools (`search_products`, `search_crm_records`, `knowledge_search`, `list_message_models`). Tool-governor limita chamadas por turno. Testes em `llm.test.ts` e `tools.test.ts`. | — |
| 7 | **Tools de efeito: add_note, send_message_model, send_product, send_whatsapp_template** | PARCIAL | `src/services/ai-v2/actions.ts` | `add_note`, `send_message_model`, `send_product`, `send_whatsapp_template` implementados e testados em `actions.test.ts`. | `send_message_model` envia só texto (sem anexos). `send_product` ignora `productPolicy` e manda resumo fixo. `send_whatsapp_template` segue a mesma lógica da v1, mas sem cache de janela de 24h. |
| 8 | **RAG: busca filtrada por tema/agente** | PRONTO E TESTADO | `src/services/ai-v2/tools.ts` `searchV2Knowledge` | Reusa `retrieveAgentKnowledge` e filtra os chunks pelos `allowedDocIds` do tema ou `allowedKnowledgeDocIds` do agente. Teste em `tools.test.ts`. | — |
| 8 | **RAG: suporte a PDF** | NÃO EXISTE | — | — | PDF não extraído; `knowledge-extract` não tocado. |
| 9 | **Modelos de mensagem: preenchimento e envio** | PARCIAL | `src/services/ai-v2/actions.ts` `executeSendMessageModel` | Carrega `MessageTemplate`, aplica variáveis (`@contact.name` etc.) e envia texto via `sendAgentMessage`. Teste em `actions.test.ts`. | Não envia anexos do modelo. A seleção continua por ação do LLM, não por RAG de modelos (a tool `list_message_models` existe mas não é obrigatória). |
...
| 10 | **Produtos: envio de resumo** | PARCIAL | `src/services/ai-v2/actions.ts` `executeSendProduct` | Busca o produto no banco e envia texto com nome, preço, descrição e campos personalizados via `sendAgentMessage`. Teste em `actions.test.ts`. | `productPolicy` por tema ainda não é aplicado (preço/condições/imagem/link não seguem flags). |
...
| 19 | **Trace: TODOS os campos do SPEC 3.24** | PARCIAL | `src/services/ai-v2/log.ts` `logV2Turn` | Grava `owner`, `stage`, `themeId`, `appliedRuleId`, `crmContext`, `prompt`, `llmOutput`, `executedActions`, `discardedActions`, `reply`, `handoff`, `closed`, `latencyMs`, `inputTokens`, `outputTokens`, `versionId`, `toolCalls`, `governorStats`. | Faltam: `previousOwner`, `reason` (actor/motivo da troca de dono), trechos RAG, `buttonId`/`buttonLabel`, custo real (`costUsd` computado mas não persistido), warnings do output guard. |
...
1. **Wizard do frontend (item 21)** — A tela é só JSON bruto. Nenhuma das 12 telas de configuração existe; administrador precisa editar `simpleConfig` manualmente.
2. **Transferência entre agentes e destinos completos (item 12)** — `handoff.ts` só aceita `department`. `ai_agent`, `user`, `distribution_rule`, `automation` ignorados. Não há controle de idas/voltas.
3. **Mensagens interativas ponta a ponta (item 11)** — Montagem e resolução existem, mas ninguém persiste, envia ou recebe o clique. Botões/listas não chegam ao cliente.
4. **Onboarding persistente e lembretes (item 20)** — Etapas existem, mas o estado não é carregado/salvo. Lembretes e “entrega final” não implementados.
5. **Custo/teto de tokens e pesquisa de satisfação (itens 17/18)** — `checkV2CostCap` e `buildSurveyMessage` existem, mas **nunca são chamados** no motor.
6. **Encerramento completo: origem da etapa + automação (item 16)** — `returnToOriginStage` e `continueV2AutomationOnClose` não são usados.
7. **Mídia: transcrição/OCR (item 4)** — Config aceita `transcribe`/`describe`, mas nenhum processamento real ocorre.
8. **Horário de atendimento / feriados / out_of_hours (item 5)** — `withinBusinessHours` é hardcoded `true`; regra `out_of_hours` nunca dispara.
9. **PDF no RAG (item 8)** — `knowledge_search` funciona em docs já indexados, mas extração de PDF ainda não existe.
10. **Produtos por política de tema (item 10)** — `send_product` envia resumo fixo; não respeita `productPolicy` (preço/condições/imagem/link).
...
**Testes de tools / function calling (`src/services/ai-v2/__tests__/tools.test.ts`, `llm.test.ts`, `actions.test.ts`)**
- `searchV2Products` (busca tolerante por nome/SKU/desc/campos)
- `searchV2CrmRecords` (contato + deal do escopo atual)
- `searchV2Knowledge` filtrado por `allowedDocIds`
- `listV2MessageModels` filtrado por `allowedIds`
- Tools passadas ao LLM, capturadas no trace e limitadas pelo tool-governor
- Allowlist de tools por tema
- Resposta do LLM renderiza `@contact.name`
- `add_note`, `send_message_model`, `send_product` executam e enviam mensagem
...
- `send_whatsapp_template` ponta a ponta (código reusa v1, mas sem teste).
