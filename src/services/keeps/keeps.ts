...
export async function listKeepNotes(opts: {
  ...
  const usedColors = usedRows
    .map((r) => r.color)
    .filter((c): c is KeepNoteColorId => c != null && isKeepNoteColor(c));
...
export async function createKeepNote(opts: {
  ...
  const category = await assertOwnedCategory({
    userId: opts.userId,
...
export async function updateKeepNote(opts: {
  ...
  if (opts.categoryId !== undefined) {
    ...
    color = category?.color ?? null;
  } else if (!existing.categoryId && opts.color !== undefined) {
    color = opts.color;
  }
...
export async function reorderKeepNotes(opts: {
  ...
  let colorByCategory = new Map<string, string>();
  if (categoryIds.length > 0) {
    ...
    colorByCategory = new Map(owned.map((c) => [c.id, c.color]));
  }
...
export async function updateKeepCategory(opts: {
  ...
  const updated = await prisma.keepCategory.update({
    where: { id: existing.id },
    data: {
      ...(name !== undefined ? { name } : {}),
  ...
  if (opts.color !== undefined && opts.color !== existing.color) {
...
export async function deleteKeepCategory(opts: { userId: string; id: string }) {
  ...
  await prisma.$transaction([
    prisma.keepNote.updateMany({
      where: { userId: opts.userId, categoryId: existing.id },
      data: { categoryId: null, color: null },
    }),
