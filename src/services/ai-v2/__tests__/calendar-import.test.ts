...
describe("importCalendarText", () => {
  ...
  it("texto já em linhas: lê direto, sem chamar o modelo", async () => {
    const r = await importCalendarText({ text: "19/10/2026 – Liberação de resultados\n02/10/2026 a 05/10/2026 – Evento", defaultYear: 2026, model: "m", apiKey: "k" });
  ...
  it("texto bagunçado (ou PDF): o modelo organiza, em modo JSON", async () => {
    gen.mockResolvedValue({ text: JSON.stringify({ events: [{ start: "2026-10-19", title: "Liberação de resultados" }, { start: "2026-11-27", end: "2026-11-30", title: "Evento" }] }) });
    const r = await importCalendarText({ text: "OUTUBRO\n19 Liberação de resultados\nNOVEMBRO\n27 a 30 Evento", defaultYear: 2026, model: "m", apiKey: "k" });
  ...
  it("resposta inválida do modelo vira erro claro", async () => {
    ...
    await expect(importCalendarText({ text: "OUTUBRO\n19 resultados", defaultYear: 2026, model: "m", apiKey: "k", forceAi: true })).rejects.toThrow(/dd\/mm\/aaaa/);
  });
});
