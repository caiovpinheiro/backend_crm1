  ...
  it("lê os formatos de data comuns e junta a continuação da linha", () => {
    const { events, unparsed } = parseCalendarText(
      [
        ...
        "== OUTUBRO de 2026 ==",
        "02/10/2026 a 05/10/2026 – Realização do Evento A",
        "19/10/2026 – Liberação de Resultados",
        "planilhas e relatórios via portal",
        "11 e 12/12 – Encerramento",
        "27 a 30 de novembro de 2026: Mutirão de novembro e dezembro",
        "5 de janeiro de 2027 - Início do atendimento",
        "18/11/26 a 14/02/27 – Período de renovação",
      ].join("\n"),
      2026,
    );
    expect(events.map((e) => [e.start, e.end ?? null, e.title])).toEqual([
      ["2026-10-02", "2026-10-05", "Realização do Evento A"],
      ["2026-10-19", null, "Liberação de Resultados planilhas e relatórios via portal"],
      ["2026-12-11", "2026-12-12", "Encerramento"],
      ["2026-11-27", "2026-11-30", "Mutirão de novembro e dezembro"],
      ["2027-01-05", null, "Início do atendimento"],
      ["2026-11-18", "2027-02-14", "Período de renovação"],
    ]);
    expect(unparsed).toEqual(["Linha de abertura sem data"]);
  });

  it("período com \"e\" entre datas completas", () => {
    const { events } = parseCalendarText("11/12/2026 e 12/12/2026 – Realização do Evento B", 2026);
    expect(events).toEqual([{ id: expect.any(String), start: "2026-12-11", end: "2026-12-12", title: "Realização do Evento B" }]);
  });
...
describe("situação e seção do prompt", () => {
  const events = [
    { id: "1", start: "2026-09-11", end: "2026-09-14", title: "Evento de agosto" },
    { id: "2", start: "2026-09-24", title: "Evento de hoje" },
    { id: "3", start: "2026-09-20", end: "2026-09-30", title: "Período de inscrição" },
    { id: "4", start: "2026-10-19", title: "Liberação de resultados" },
  ...
  it("monta a seção em ordem, com a situação, sem o que é muito antigo", () => {
    ...
    expect(s).toContain("- 11/09/2026 a 14/09/2026 — Evento de agosto (já passou)");
    ...
    expect(s).toContain("- 19/10/2026 — Liberação de resultados (próximo)");
    expect(s).not.toContain("Muito antigo");
    expect(s.indexOf("Evento de agosto")).toBeLessThan(s.indexOf("Liberação de resultados"));
  });
