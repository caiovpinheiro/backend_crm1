 ...
 * smart. A seleção é sorteio: cada slot ativo (`slotIndex < weight`) é um
 * bilhete. Peso 5 vs 1 = 5 nomes na urna vs 1; a fila e o `lastAssignedAt`
 * não escolhem o ganhador. Transaction por org (advisory lock) só serializa
 * o claim — o CAS do alvo protege a corrida no item.
 *
...
/** Sorteio: cada slot ativo (slotIndex < weight) é um bilhete. */
...
export async function executeLeadsDistribution(
  ...
  try {
    txOutcome = await prisma.$transaction(async (tx) => {
      // Serializa o claim por organização (duas atribuições no mesmo instante).
      await tx.$executeRaw`
