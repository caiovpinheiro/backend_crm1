...
export async function ensureConversationInWaitingQueue(args: {
  ...
  if (
    (convRoute as { routeMode?: string | null } | null)?.routeMode === "leads" ||
    (convRoute as { department?: { distributionMode?: string } | null } | null)?.department?.distributionMode === "leads"
  ) {
    return;
  }
...
/**
 * Outbound com `userId`: se o consultor tem vaga, limpa fruitless e
 * deixa o drain seguir. Sem isso a Retenção (único membro no teto)
 * ficava congelada até o Reprocessar — o `capacity_released` dele
 * era skip enquanto o Redis `dist:fruitless:{org}` permanecia armado.
 */
